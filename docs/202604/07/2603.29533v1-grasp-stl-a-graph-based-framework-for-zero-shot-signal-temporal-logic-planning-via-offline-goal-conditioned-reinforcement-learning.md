...
tags: ["query:q7"]
score: 6.0
evidence: STL规范下的零样本规划
tldr: 本研究针对离线零样本信号时序逻辑（STL）规划问题，提出了GraSP-STL框架。该方法仅利用任务无关的离线数据集，无需环境交互或任务重训，通过学习目标条件价值函数构建状态空间的有向图抽象。利用图搜索寻找满足STL规范的路标点序列，并由目标条件策略执行，实现了对未知复杂任务的零样本泛化和长时程规划。
source: arxiv
...
method: 提出一种基于图搜索的框架，通过学习目标条件价值函数构建可达性度量图，并结合算术几何平均鲁棒性进行路径规划与执行。
result: 实验证明该框架在多种离线STL规划任务中表现出色，能够有效组合短时程行为以完成长时程复杂任务。
conclusion: GraSP-STL成功分离了可达性学习与任务规划，为离线强化学习在复杂逻辑约束下的零样本泛化提供了有效方案。
---
