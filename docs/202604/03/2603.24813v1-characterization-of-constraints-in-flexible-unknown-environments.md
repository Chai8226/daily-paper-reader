...
tags: ["query:q6"]
score: 6.0
evidence: 未知环境中的在线路径规划
tldr: 本研究提出一种在线路径规划算法，用于在未知环境中安全操纵具有柔性约束的物体。通过实时识别感知到的柔性约束和全局刚度，机器人能够同步进行探索、特征提取与安全操作。该方法利用局部力与位置信息进行约束探索，并通过刚度特征向量识别合页或平面等机械约束参数，为手术中器官牵引等复杂应用提供了可行方案。
source: arxiv
...
motivation: 旨在解决机器人在未知环境中如何安全、自主地操纵具有柔性约束物体这一难题。
method: 提出一种结合局部力与位置信息探索与全局刚度特征向量分析的在线路径规划和约束识别算法。
result: 仿真和实验证明该算法能实时识别合页等常见机械约束并准确提取相关的螺旋参数。
conclusion: 该方法实现了柔性约束物体的同步探索与安全操纵，在医疗手术等需要精细操作的领域具有重要应用潜力。
---

## 摘要
本文提出了一种在线路径规划算法，用于在未知环境中对柔性约束物体进行安全的自主操作。文中介绍了实时识别和表征感知到的柔性约束及全局刚度的方法。通过协同使用，这些方法允许机器人在安全的前提下，同时对弹性系统进行探索、表征和操作。利用基于局部力和位置信息的约束探索，实现了在无系统先验知识情况下的导航。在物体（系统）轨迹的多个位姿上考虑感知到的约束刚度。利用刚度特征向量信息，并通过简单机械约束图谱（如铰链和平面约束）来表征和识别全局刚度行为。通过仿真和实验对这些算法进行了验证。结果表明，该算法能够实时识别几种常见的简单机械约束（如柔性铰链），并随后识别相关的螺旋参数。这些结果证明了同时进行全局约束/刚度探索和柔性约束物体安全操作的可行性。我们相信，这种方法最终将能够在手术过程中的器官牵开与操作等应用中实现安全的协作操作。
