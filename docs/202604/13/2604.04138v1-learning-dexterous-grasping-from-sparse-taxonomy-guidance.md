...
tags: ["query:q3"]
score: 6.0
evidence: 协调多指控制与抓取规划
tldr: "针对灵巧手抓取中密集姿态标注难和端到端强化学习缺乏可控性的挑战，本文提出GRIT框架。该框架采用两阶段方法：首先根据场景和任务预测稀疏的抓取分类（Taxonomy）规范，随后策略根据该指令生成连续手指动作。GRIT利用抓取分类与物体几何的关联性，显著提升了对新物体的泛化能力，在实验中达到87.9%的成功率，并支持通过高层指令灵活调整抓取策略。"
source: arxiv
...
motivation: 传统的灵巧操作研究面临密集姿态标注成本高和端到端强化学习缺乏可控性且难以干预的问题。
method: 提出两阶段框架GRIT，先从环境预测稀疏的抓取分类指令，再由策略驱动手指完成符合该分类的动作。
result: "该方法在多样化物体上实现了87.9%的抓取成功率，并在真实世界中展现了出色的可控性与泛化性能。"
conclusion: 引入稀疏的抓取分类指导是实现高效、可控且具有泛化能力的灵巧手操作的有效途径。
---
