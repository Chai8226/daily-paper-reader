...
title_zh: GraSP-STL：基于离线目标条件强化学习的零样本信号时序逻辑规划图框架
authors: "Ancheng Hou, Ruijia Liu, Xiang Yin"
date: 2026-03-31
pdf: "https://arxiv.org/pdf/2603.29533v1"
tags: ["query:q7"]
score: 6.0
evidence: 时序逻辑规范下的零样本规划
tldr: 本研究提出GraSP-STL框架，旨在解决离线数据集下的零样本信号时序逻辑（STL）规划问题。该方法无需环境交互或任务重训，通过从离线数据中学习目标条件价值函数并构建有向图抽象，将复杂STL任务转化为图搜索问题。该框架实现了可重用的可达性学习与任务规划的分离，支持长时程规划并能泛化至未见过的STL规范。
source: arxiv
...
motivation: 针对缺乏动力学模型且无法进行环境交互的离线场景，实现对任意未知STL规范的零样本规划。
method: 结合离线目标条件强化学习与图搜索，利用学习到的可达性度量构建状态图，并通过算术几何平均鲁棒性评估路径。
result: 实验证明该框架在多种离线STL规划任务中表现出色，能够有效组合短时程行为以完成长时程任务。
conclusion: GraSP-STL成功分离了可达性学习与任务规划，为离线零样本STL规划提供了一种高效且泛化能力强的解决方案。
---

## 摘要
本文研究了信号时序逻辑（STL）规范下的离线零样本规划。我们假设仅能访问由任务无关的行为策略收集的状态-动作-状态转移离线数据集，且没有解析动力学模型，不进行进一步的环境交互，也不进行特定任务的重新训练。目标是合成一种控制策略，使其产生的轨迹满足任意未见的 STL 规范。为此，我们提出了 GraSP-STL，这是一个基于图搜索的框架，用于从离线轨迹中进行零样本 STL 规划。该方法从离线数据中学习目标条件价值函数，并利用其在状态空间上导出有限时域可达性度量。基于该度量，它构建了一个有向图抽象，其节点代表代表性状态，边编码了可行的短时域转移。规划随后被表述为对路标点序列的图搜索，使用算术-几何平均鲁棒性及其区间语义进行评估，并由学习到的目标条件策略执行。所提出的框架将可重用的可达性学习与任务条件规划分离，通过组合离线数据中的短时域行为，实现了对未见 STL 任务的零样本泛化和长时域规划。实验结果证明了该框架在一系列离线 STL 规划任务中的有效性。
