...
title_zh: 用于平滑可微且高效向量化接触流形构建的新型算法
authors: "Onur Beker, Andreas René Geist, Anselm Paulus, Georg Martius"
date: 2026-04-19
pdf: "https://arxiv.org/pdf/2604.17538v1"
tags: ["query:q4"]
score: 8.0
evidence: 用于接触丰富机器人行为的可微碰撞检测
tldr: 针对接触密集型机器人行为生成中一阶/二阶导数获取困难的问题，本文提出了一种平滑可微且可大规模向量化的碰撞检测方法。该方法通过引入一类高表达能力的解析符号距离函数（SDF）原语来表示复杂3D表面，并结合新型接触流形生成程序，有效解决了仿真管线中的梯度计算瓶颈，显著提升了计算效率。
source: arxiv
...
motivation: 现有的零阶优化方法效率较低，而利用高阶导数信息提升动力学求解速度面临碰撞检测不可微的挑战。
method: 提出了一类高效表达复杂几何的解析SDF原语，并开发了配套的平滑可微接触流形生成算法。
result: 实现了支持大规模向量化计算且具备平滑梯度的碰撞检测流程，克服了传统仿真管线中的梯度病态问题。
conclusion: 该研究为利用高阶导数优化机器人接触动力学仿真和行为生成提供了高效且可扩展的技术方案。
---

## 摘要
在富接触场景中生成智能机器人行为是一个目前零阶方法占据主导地位的研究问题。开发利用动力学一阶/二阶信息的方法，在提高求解速度和计算效率方面具有巨大潜力。该研究方向的主要瓶颈在于难以获得有用的梯度和 Hessian 矩阵，这是由于通用仿真流水线的所有三个步骤中都存在病态问题：i) 碰撞检测，ii) 接触动力学，iii) 时间积分。本摘要提出了一种能够以平滑可微且大规模向量化的方式解决碰撞检测环节难题的方法。这是通过两项贡献实现的：i) 一类具有高度表达能力的解析 SDF 原语，可以高效地表示复杂的 3D 表面；ii) 一种利用该几何表示的新型接触流形生成程序。
