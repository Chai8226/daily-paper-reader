...
tags: ["query:q3"]
score: 6.0
evidence: 移动操作行为与高级语言引导规划器
tldr: 针对移动机器人在人机交互中缺乏意图表达及应对实时干扰能力的问题，本文提出ExpressMM框架。该框架结合了基于视觉语言模型（VLM）的高层规划器与底层视觉-语言-动作（VLA）策略，旨在生成具有表现力的机器人行为。ExpressMM支持用户在任务执行中进行中断或重定向指令，通过协作装配场景的评估证明，该系统显著提升了机器人意图的可解释性、社交适应性及安全性。
source: arxiv
selection_source: fresh_fetch
motivation: 现有的机器人表达性行为研究大多忽略了人机交互中用户可能实时中断、修改或重定向机器人动作的需求。
method: 开发了ExpressMM框架，通过集成VLM高层规划器与底层VLA策略，实现支持实时中断和表达性行为的移动操作。
result: 问卷调查显示，ExpressMM生成的行为能让观察者清晰理解机器人意图，并使其在协作任务中表现得更安全、可预测。
conclusion: 该框架有效增强了移动机器人在复杂人机协作环境中的沟通能力、任务灵活性和用户接受度。
---

## 摘要
移动操作机器人越来越多地被部署在以人为中心的环境中执行任务。在完成这些任务的同时，它们还应该能够通过表达性机器人行为向周围的人传达其意图。先前关于表达性机器人行为的研究采用了预编程或基于示教学习的表达性动作，以及由大语言模型生成的高层交互。现有的大多数方法尚未考虑用户可能在任务执行期间中断、修改或重定向机器人动作的人机交互（HRI）场景。在本文中，我们开发了全新的 ExpressMM 框架，该框架将基于视觉语言模型（用于感知和对话推理）的高层语言引导规划器与底层视觉-语言-动作策略相结合，以在协作式 HRI 任务中生成表达性机器人行为。此外，ExpressMM 支持可中断的交互，以适应用户更新或重定向的指令。我们在移动操作机器人协助人类进行协作装配的场景中演示了 ExpressMM，并对现场 HRI 演示进行了基于观众的评估。问卷结果显示，由 ExpressMM 赋能的表达性行为有助于观察者清晰地解读机器人的动作和意图，同时支持符合社交规范且易于理解的交互。参与者还反馈称，机器人在协作任务中非常有用，且在演示过程中表现得可预测且安全，从而提升了人们对机器人在协作任务中的实用性、安全性和可预测性的正面认知。
