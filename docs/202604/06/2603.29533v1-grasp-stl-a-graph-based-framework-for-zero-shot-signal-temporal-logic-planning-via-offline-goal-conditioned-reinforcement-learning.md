...
title_zh: GraSP-STL：一种基于离线目标条件强化学习的零样本信号时序逻辑规划图框架
authors: "Ancheng Hou, Ruijia Liu, Xiang Yin"
date: 2026-03-31
pdf: "https://arxiv.org/pdf/2603.29533v1"
tags: ["query:q7"]
score: 6.0
evidence: 通过强化学习在时序逻辑规范下进行轨迹规划
tldr: 本文提出GraSP-STL框架，旨在解决离线零样本信号时序逻辑（STL）规划问题。该方法仅利用任务无关的离线数据集，无需环境模型或额外交互，通过学习目标条件价值函数构建状态空间的可达性图抽象。利用算术几何平均鲁棒性在图上搜索路径点序列，并由目标条件策略执行，实现了对未知STL任务的零样本泛化和长时程规划。
source: arxiv
...
motivation: 针对在无环境交互和无任务重训的情况下，如何利用离线数据实现对任意未知STL规范的零样本规划挑战。
method: 通过离线数据学习价值函数以构建可达性图，并结合STL鲁棒性语义在图上进行路径点搜索与策略执行。
result: 实验结果表明，该框架在多种离线STL规划任务中表现出色，能够有效处理长时程任务并泛化至未见过的逻辑规范。
conclusion: GraSP-STL通过解耦可达性学习与任务规划，为离线强化学习在复杂逻辑约束下的零样本应用提供了高效方案。
---

## 摘要
本文研究了信号时序逻辑（STL）规范下的离线零样本规划。我们假设仅能访问由任务无关的行为策略收集的状态-动作-状态转移离线数据集，且不具备解析动力学模型，无需进一步的环境交互，也不进行针对特定任务的重新训练。其目标是合成一种控制策略，使其生成的轨迹能够满足任意未见的 STL 规范。为此，我们提出了 GraSP-STL，这是一个基于图搜索的框架，用于从离线轨迹中进行零样本 STL 规划。该方法从离线数据中学习目标条件价值函数，并利用其在状态空间上导出有限时域可达性度量。基于该度量，它构建了一个抽象有向图，其节点表示代表性状态，边则编码了可行的短时域转移。随后，规划被建模为对路标点序列的图搜索，使用算术-几何平均鲁棒性及其区间语义进行评估，并由学习到的目标条件策略执行。所提出的框架将可重用的可达性学习与任务条件规划相分离，从而实现了对未见 STL 任务的零样本泛化，并通过组合离线数据中的短时域行为实现了长时域规划。实验结果证明了该框架在一系列离线 STL 规划任务中的有效性。
