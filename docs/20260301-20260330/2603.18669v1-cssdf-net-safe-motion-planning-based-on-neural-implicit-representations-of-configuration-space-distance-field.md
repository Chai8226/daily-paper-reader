...
tags: ["query:q6"]
score: 9.0
evidence: 基于构型空间距离场的安全运动规划
tldr: 针对高维机械臂在非结构化环境中的避障难题，本文提出CSSDF-Net，一种直接在配置空间（C-space）学习连续有符号距离场的神经隐式表示方法。该方法通过空间哈希数据生成流程实现了对未知环境的零次学习泛化，统一处理自碰撞与环境碰撞。实验证明其能提供稳定的梯度，支持离线轨迹优化与在线预测控制，在动态和静态场景中均表现出高效的避障能力。
source: arxiv
selection_source: fresh_fetch
figures_json: "[{\"url\": \"assets/figures/arxiv/2603.18669v1/fig-001.webp\", \"caption\": \"Fig. 4: Dynamic obstacle avoidance sequence for the 7-DoF manipulator.\", \"page\": 7, \"index\": 1, \"width\": 1026, \"height\": 220}, {\"url\": \"assets/figures/arxiv/2603.18669v1/fig-002.webp\", \"caption\": \"TABLE VI: Comparative trajectory planning performance for the 7-DoF manipulator\", \"page\": 7, \"index\": 2, \"width\": 404, \"height\": 316}, {\"url\": \"assets/figures/arxiv/2603.18669v1/fig-003.webp\", \"caption\": \"Fig. 2: Comparative analysis of trajectory optimization with safety constraints.\", \"page\": 5, \"index\": 3, \"width\": 512, \"height\": 673}]"
motivation: 现有的几何碰撞检测器不可微，且基于工作空间的隐式模型在映射到配置空间时存在非线性强、收敛性差以及自碰撞处理分离的问题。
method: 提出CSSDF-Net，利用空间哈希编码机器人几何先验，直接在配置空间学习统一的连续有符号距离场，并将其集成到轨迹优化和模型预测控制中。
result: 在平面臂和7自由度机械臂上的实验表明，该方法在未见过的静态和动态环境中均能实现稳定的梯度输出、有效的碰撞规避以及实时的推理性能。
conclusion: CSSDF-Net为高维机器人提供了一种高效、可微且具备泛化能力的配置空间距离查询机制，显著提升了复杂环境下运动规划的安全性和鲁棒性。
---

## 摘要
非结构化环境中的高维机械臂操作需要一种可微且与场景无关的距离查询机制，以引导安全运动的生成。现有的几何碰撞检测器通常是不可微的，而基于工作空间的隐式距离模型受限于高度非线性的工作空间-位形映射，且往往面临收敛性差的问题；此外，自碰撞和环境碰撞通常被作为独立的约束处理。我们提出了位形空间有向距离场网络（CSSDF-Net），它直接在位形空间中学习连续的有向距离场，在统一的几何安全概念下提供关节空间的距离和梯度查询。为了在无需针对特定环境重新训练的情况下实现零样本泛化，我们引入了一种基于空间哈希的数据生成流水线，该流水线编码了以机器人为中心的几何先验，并支持针对任意障碍物点集的高效风险位形检索。学习到的距离场被集成到安全约束轨迹优化和收缩时界 MPC 中，从而实现离线规划和在线反应式避障。在平面机械臂和 7 自由度机械臂上的实验表明，该方法具有稳定的梯度，在静态和动态场景中均能有效避障，且在大规模点云查询中具有实用的推理延迟，支持在先前未见过的环境中进行部署。
